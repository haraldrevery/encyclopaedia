...
const BUCKETS = {
  ...
  dimensions: "images whose pixel size could not be read, so a gallery guessed",
  ...
  demoted: "bodies whose own h1 was shifted below the page title, likewise",
  settings: "unusable values in site_settings.json, from library.js",
};

/**
 * Three of these — math, render and demoted — are NOT filled in through add().
 *
 * library.js hands the bare arrays to markdown.js as sinks (setMathCollector
 * and friends) and swaps them for empty ones afterwards, which is how the
 * findings from about.md and legal.md are dropped: those two pages render long
 * after the report has been counted, and letting them in would make the tile
 * and the section below it disagree depending on Eleventy's template order.
 * A keyed collector cannot express "and now stop listening", so the sinks stay.
 *
 * They do not need the deduping either: library.js renders each document
 * exactly once, in one loop, before any page is written. They are declared here
 * anyway because this list is meant to be the buckets — a name missing from it
 * is a name add() would reject, and `demoted` was exactly that.
 */
