...
const { relTo, isImage, isVideo, isAudio, isMarkdown, videoMime, audioMime } = require("./paths");
...
function expandWikiEmbeds(source, pageUrl, mediaUrl) {
  return source.replace(/!\[\[([^\]|]+?)(?:\|[^\]]*)?\]\]/g, (whole, target) => {
...
function rewriteRelativeImages(source, pageUrl, mediaUrl) {
  return source.replace(/!\[([^\]]*)\]\(\s*([^)\s]+)([^)]*)\)/g, (whole, alt, src, tail) => {
...
/** Give every h2/h3 an id, respecting one the author already wrote. */
...
function toc(html) {
  if (!html) return "";
  const headings = [];
  const re = /<h([23])\b[^>]*\sid\s*=\s*"([^"]*)"[^>]*>([\s\S]*?)<\/h\1>/gi;
  let match;
...
module.exports = {
  md, render, addAnchors, toc,
  setMathCollector, setRenderCollector,
  videoTag, audioTag,
};
