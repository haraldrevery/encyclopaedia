...
const { imageDimensions } = require("./dimensions");
...
function collectMedia(dir, relPath, kinds, mediaUrl, health, dims) {
  ...
  return {
    ...
    images: kinds.images.map((f) => {
      ...
      // One stat, three uses: the KB label, the dimension cache key, and the
      // size reported alongside a dimension failure.
      const stat = fileStat(abs);
      const size = imageDimensions(abs, stat);
      const file = relPathOf(relPath, f);
      if (!size) {
        // Not fatal and not silent. markdown.js lays the image out at a
        // declared fallback ratio, which is the right thing for the page and
        // invisible to the author — so it is a row on the status page instead.
        health.add("dimensions", file, { file, url: encode(f) });
      } else if (dims) {
        // Keyed by path inside input_markdown/, because a body may embed an
        // image from a sibling folder and markdown.js resolves what the author
        // WROTE against the document's own directory.
        dims.set(file, size);
      }
      return {
        filename: f,
        ...
        // null, not a guessed ratio: the fallback belongs to the layout that
        // needs one, not to the model. media.njk ignores both.
...
function walk(dir, relPath, depth, ancestors, siblingSlugs, ctx) {
  ...
  node.media = collectMedia(dir, relPath, kinds, mediaUrl, ctx.health, ctx.dims);
...
function scan(rootDir, options = {}) {
  ...
  const ctx = { nodes: [], entries: [], variants: [], health, perPage, dims: new Map() };
  ...
  return { root, nodes: ctx.nodes, entries: ctx.entries, variants: ctx.variants, health, dims: ctx.dims };
}
