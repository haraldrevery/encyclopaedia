...
const {
  render, addAnchors, toc, bodyImages, imageKey,
...
module.exports = function () {
  ...
  for (const variant of result.variants) {
    ...
    // The folder's images minus the ones this document already shows, so the
    // "Images" block under an entry is what the page has NOT displayed rather
    // than a second copy of it.
    //
    // Computed per variant and stored on the variant, never by filtering
    // folder.media.images: that array is shared by every entry in the folder,
    // so editing it would strip a photograph from a sibling page that does not
    // embed it. `folder` here is the flat per-variant copy scan.js makes, but
    // `folder.media` inside it is still a reference to the one shared object.
    // imageKey() rather than a hand-built path: the set bodyImages() returns
    // was keyed with that exact function, and two spellings of "the same file"
    // is how a filter like this silently stops matching.
    const shown = bodyImages();
    const dir = path.posix.dirname(variant.relPath);
    variant.unusedImages = (variant.folder.media.images || [])
      .filter((image) => !shown.has(imageKey(dir, image.filename)));
  }
...
function buildHealthReport(result) {
  ...
  const sections = [
    ...
    // Low: the image is still shown and still opens in the lightbox — only its
    // slot in a justified run is a guess, and a guessed slot is a cosmetic
    // wrong shape rather than anything missing. Guarded like the two above,
    // because an older scan() result has no such list.
    { key: "dimensions", label: "Unmeasured images", count: (health.dimensions || []).length, severity: "low",
      note: "images laid out at a guessed aspect ratio" },
