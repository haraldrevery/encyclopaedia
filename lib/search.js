...
const { NO_TITLE } = require("./normalise");

/** Keys are one letter because the whole file ships on the first search. */
...
function buildSearchIndex(entries) {
  ...
  return entries.map((entry) => {
    ...
    // The same fallback collapsedCard() applies in scan.js, and for the same
    // reason: a collapsed folder IS its entry, so the folder name is the name
    // the reader saw on the card they are trying to find again. Without it an
    // untitled document was indexed as the literal "-no title-" — unfindable
    // under the only name the site had ever shown for it. Not applied when the
    // folder is not collapsed: there the card says "-no title-" too, and the
    // index has to match what is on screen rather than improve on it.
    const title =
      entry.title === NO_TITLE && folder && folder.collapsed
        ? folder.label
        : entry.title;
